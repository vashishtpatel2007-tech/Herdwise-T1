const fs = require('fs');
let content = fs.readFileSync('scratch/dashboard.html', 'utf8');

const oldFunc = `function distanceToSegment(lat, lon, lat1, lon1, lat2, lon2) {
  const R = 6371000;
...
const newFunc = `function distanceToSegment(lat, lon, lat1, lon1, lat2, lon2) {
  const R = 6371000;
  const x = 0; const y = 0;
...
fs.writeFileSync('scratch/dashboard.html', content);
